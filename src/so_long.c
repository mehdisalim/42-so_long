...
/*   Updated: 2022/12/09 17:46:57 by esalim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void setup_background(t_data mlx, int height, int width)
{
	...
	while ((x += 50) < height)
	{
		y = -50;
		while ((y += 50) < width)
			mlx_put_image_to_window(mlx.ptr, mlx.win, mlx_xpm_file_to_image(mlx.ptr, "./assets/background/bg.xpm", &s, &s), x, y);
	}
}
...
t_data	init(char *filename)
{
	...
	t_data	mlx;
 	...
 	mlx.ncoins = 0;
 	int a = 50;
 	mlx.playerimg = mlx_xpm_file_to_image(mlx.ptr, "./assets/P/elf0.xpm", &a, &a);
 	getdoor_coord(&mlx);
 	return (mlx);
}
